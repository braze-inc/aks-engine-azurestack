...
---
{{/* Source: azuredisk-csi-driver/templates/csi-azuredisk-node-windows-hostprocess.yaml */}}
kind: DaemonSet
apiVersion: apps/v1
metadata:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      initContainers:
        - name: init
          image: {{ContainerImage "azuredisk-csi-windows-hp"}}
          imagePullPolicy: IfNotPresent
          command:
            - "powershell.exe"
            - "-c"
...
{{- end}}
          ...
          resources:
            limits:
              cpu: {{ContainerCPULimits "azuredisk-csi-windows-hp"}}
              memory: {{ContainerMemLimits "azuredisk-csi-windows-hp"}}
            requests:
              cpu: {{ContainerCPUReqs "azuredisk-csi-windows-hp"}}
              memory: {{ContainerMemReqs "azuredisk-csi-windows-hp"}}
          securityContext:
            capabilities:
              drop:
                - ALL
